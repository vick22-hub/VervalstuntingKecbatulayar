...
<script type="module">
  ...
  // 2. GANTI BAGIAN INI DENGAN FIREBASE CONFIG ANDA DARI LANGKAH 2!
...
npm install firebase
  ...
  // Mengecek apakah ada yang sedang login
  onAuthStateChanged(auth, (user) => {
    if (user) {
      statusTeks.innerText = "Selamat datang, " + user.email;
  ...
  // Aksi ketika tombol "Daftar Akun Baru" diklik
  btnDaftar.addEventListener('click', () => {
    createUserWithEmailAndPassword(auth, emailInput.value, passwordInput.value)
      .then((userCredential) => { alert("Pendaftaran Berhasil! Anda otomatis masuk."); })
      .catch((error) => { alert("Gagal Daftar: " + error.message); });
  });

  // Aksi ketika tombol "Masuk (Login)" diklik
  btnLogin.addEventListener('click', () => {
    signInWithEmailAndPassword(auth, emailInput.value, passwordInput.value)
      .then((userCredential) => { alert("Login Berhasil!"); })
      .catch((error) => { alert("Gagal Login: Cek kembali email dan password Anda."); });
  });

  // Aksi ketika tombol "Keluar (Logout)" diklik
  btnLogout.addEventListener('click', () => {
    signOut(auth).then(() => { alert("Anda telah keluar."); });
  });
</script>
